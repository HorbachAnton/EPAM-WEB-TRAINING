		...
		<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt"%>
			...
			<html lang="en">
				...
				<body>
					...
					<fmt:message bundle="${loc}" key="locale.order.cap_id" var="cap_id"/>
					<fmt:message bundle="${loc}" key="locale.order.cap_title" var="cap_title"/>
					<fmt:message bundle="${loc}" key="locale.order.cap_type" var="cap_type"/>
					<fmt:message bundle="${loc}" key="locale.order.cap_description" var="cap_description"/>
					<fmt:message bundle="${loc}" key="locale.order.cap_dosage" var="cap_dosage"/>
					<fmt:message bundle="${loc}" key="locale.order.cap_need_recipe" var="cap_need_recipe"/>
						...
						<c:forEach var="medicine" items="${listMedicines}">
							<div class="row d-flex align-items-center justify-content-center border border-secondary row_medicine">
								...
								<form action="contoller" method="post">
									...
									<input type="submit" class="btn btn-secondary" value="Удалить из заказа"/>
								</form>
							</div>
						</c:forEach>
						<form class="d-flex align-items-center justify-content-center" action="contoller" method="go_to_user_page">
							<input type="hidden" name="command" value="go_to_user_page"/>
							<input type="submit" class="btn btn-secondary" value="Вернуться назад"/>
						</form>
						<form class="d-flex align-items-center justify-content-center" action="contoller" method="go_to_user_page">
							<input type="hidden" name="command" value="add_order"/>
							<input type="submit" class="btn btn-secondary" value="Оформить заказ"/>
						</form>
					</div>
