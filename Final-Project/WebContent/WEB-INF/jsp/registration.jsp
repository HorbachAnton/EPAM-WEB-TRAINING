  ...
  <%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt"%>
    ...
    <html lang="en">
      ...
      <body>
        ...
        <fmt:message bundle="${loc}" key="locale.registration.email_message" var="email_message"/>
        <fmt:message bundle="${loc}" key="locale.registration.email_description" var="email_description"/>
        <fmt:message bundle="${loc}" key="locale.registration.password_message" var="password_message"/>
        <fmt:message bundle="${loc}" key="locale.registration.password_description" var="password_description"/>
        <fmt:message bundle="${loc}" key="locale.registration.enter_email" var="enter_email"/>
        <fmt:message bundle="${loc}" key="locale.registration.enter_email_help" var="enter_email_help"/>
        <fmt:message bundle="${loc}" key="locale.registration.enter_password" var="enter_password"/>
                ...
                <div class="col-sm-4 border border-secondary rounded-lg registration" style="background:cornsilk;">
                  <form class="text-center" action="contoller" method="post">
                    ...
                    <div class="form-group">
                      ...
                      <input type="password" name="password" class="form-control" id="exampleInputPassword1" placeholder="Password" required="required" pattern="((?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%]).{6,20})"/>
                    </div>
